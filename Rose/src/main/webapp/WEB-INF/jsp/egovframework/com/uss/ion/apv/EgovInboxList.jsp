			...
			<div class="content_box"> 
				...
				<div class="title_box">
					<div class="title_line">
						<h1><spring:message code="appvl.sidemenu.tree.cabinet.inbox"/></h1>						
					</div>					
				</div>
				...
				<!-- List Start -->				
					<table summary="/No/Attochment/Title/Poster/Post Date/Expire date" class="board_type_height">
						...
						<thead>
							<tr>
								<th scope="col" onclick="javascript:fnOrderList(this, 1);" class='<c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.type"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 2);" class='<c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.approvaldate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 3);" class='<c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.finisheddate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 4);" class='<c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.title"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 5);" class='<c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.sender"/></span></th>	
								<th scope="col" onclick="javascript:fnOrderList(this, 6);" class='<c:if test="${page.orderColumn eq '6' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '6' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.draftdept"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 7);" class='<c:if test="${page.orderColumn eq '7' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '7' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.inbox.table.drafter"/></span></th>
						...
						<c:if test="${empty inboxList}">
							<tr><td colspan="9" class="table_nodata"><span><spring:message code="common.msg.table.nodata"/></span></td></tr>
						</c:if>						
						<c:forEach var="inbox" items="${inboxList}">
							<tr>
								<td>
									<c:if test="${inbox.docType == 'DT01'}"><spring:message code="appvl.doctype.internal"/></c:if>
									<c:if test="${inbox.docType == 'DT02'}"><spring:message code="appvl.doctype.outgoing"/></c:if>
									<c:if test="${inbox.docType == 'DT03'}"><spring:message code="appvl.doctype.incoming"/></c:if>
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${inbox.approvedDateTime }" />
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${inbox.draftDateTime }" />
								</td>
								<td><a href="javascript:approvalView('<c:out value="${inbox.docID}"/>')">${inbox.docTitle}</a></td>
								<td>${inbox.senderName}</td>
								<td>${inbox.draftDeptName}</td>
								<td>${inbox.drafterName}</td>
								<td class="align_center">
								<a href="javascript:status('<c:out value="${inbox.docID}"/>', '<c:out value="${inbox.docTitle}"/>')" class="Status">
									<input type="hidden" value="${inbox.docID}" name="ongoingDocId">
									<img src="<c:url value='/images/egovframework/com/uss/cmm/icon_history.png'/>" alt="icon_history">
								</a>
								</td>
								<td class="align_center">
									<c:if test="${inbox.docAttaF =='1'}">
										<a href="javascript:fileDown('<c:out value="${inbox.docID}"/>', '<c:out value="${inbox.docTitle}"/>')" class="fileDown">										
											<input type="hidden" value="${inbox.docID}" id="ongoingDocId" name="ongoingDocId">
