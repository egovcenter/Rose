			...
			<div class="content_box"> 
				...
				<div class="title_box">
					<div class="title_line">
						<h1><spring:message code="appvl.document.title.label.myappvldocum"/></h1>						
					</div>					
				</div>
				...
				<!-- List Start -->							
					<table summary="/No/Attochment/Title/Poster/Post Date/Expire date" class="board_type_height">
						...
						<c:forEach var="approval" items="${approvalList}">
							<tr>
								<td>
									<c:if test="${approval.docType == 'DT01'}"><spring:message code="appvl.doctype.internal"/></c:if>
									<c:if test="${approval.docType == 'DT02'}"><spring:message code="appvl.doctype.outgoing"/></c:if>
									<c:if test="${approval.docType == 'DT03'}"><spring:message code="appvl.doctype.incoming"/></c:if>
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${approval.draftDateTime }" />
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${approval.approvedDateTime }" />
								</td>
								<td><a href="javascript:approvalView('<c:out value="${approval.docID}"/>')"> ${approval.docTitle} </a></td>
								<td>${approval.draftDeptName}</td>
								<td>${approval.drafterName}</td>
								<td class="align_center">
									<a href="javascript:status('<c:out value="${approval.docID}"/>', '<c:out value="${approval.docTitle}"/>')" class="Status">
									<img src="<c:url value='/images/egovframework/com/uss/cmm/icon_history.png'/>" alt="icon_history"></a>
								</td>
								<td class="align_center">
									<c:if test="${approval.docAttaF =='1'}">
										<a href="javascript:fileDown('<c:out value="${approval.docID}"/>', '<c:out value="${approval.docTitle}"/>')" class="fileDown">
											<input type="hidden" value="${approval.docID}" id="approvalDocId" name="approvalDocId">
