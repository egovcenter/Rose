			...
			<div class="content_box"> 
				...
				<div class="title_box">
					<div class="title_line">
						<h1><spring:message code="appvl.sidemenu.tree.cabinet.passbox"/></h1>						
					</div>					
				</div>
				...
				<!-- List Start -->		
					<table summary="/No/Attochment/Title/Poster/Post Date/Expire date" class="board_type_height">
						...
						<thead>
							<tr>
								<th scope="col" onclick="javascript:fnOrderList(this, 1);" class='<c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.draftdate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 2);" class='<c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.finisheddate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 3);" class='<c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.title"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 4);" class='<c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.draftdept"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 5);" class='<c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.drafter"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 6);" class='<c:if test="${page.orderColumn eq '6' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '6' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.passbox.table.pass"/></span></th>
						...
						<tbody>
							<c:if test="${empty passboxList}">
								<tr><td colspan="8" class="table_nodata"><span><spring:message code="common.msg.table.nodata"/></span></td></tr>
							</c:if>						
							<c:forEach var="passbox" items="${passboxList}">
							<tr>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${passbox.draftDateTime }" />
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${passbox.approvedDateTime }" />
								</td>
								<td><a href="javascript:approvalView('<c:out value="${passbox.docID}"/>')">${passbox.docTitle}</a></td>
								<td>${passbox.draftDeptName}</td>
								<td>${passbox.drafterName}</td>
								<td>${passbox.recpDeptNames}</td>
								<td class="align_center">
									<a href="javascript:status('<c:out value="${passbox.docID}"/>', '<c:out value="${passbox.docTitle}"/>')" class="Status">
									<img src="<c:url value='/images/egovframework/com/uss/cmm/icon_history.png'/>" alt="icon_history"></a>
								</td>
								<td class="align_center">
									<c:if test="${passbox.docAttaF =='1'}">
										<a href="javascript:fileDown('<c:out value="${passbox.docID}"/>', '<c:out value="${passbox.docTitle}"/>')" class="fileDown">										
											<input type="hidden" value="${passbox.docID}" id="ongoingDocId" name="ongoingDocId">
...
function status(docId, docTitle){
	...
	$(".listSignerTr").remove();
	var url = "<c:url value='/listApprovalRecipient.do'/>";
	$.ajax({                        
        type: "POST",
        url: url,
        dataType: 'html',
