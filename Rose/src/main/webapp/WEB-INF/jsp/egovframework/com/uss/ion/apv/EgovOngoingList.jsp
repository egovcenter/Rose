...
<style type="text/css">
	.rapper_table {margin-bottom:0 !important;}
</style>
</head>
			...
			<div class="content_box"> 
				...
				<div class="title_box">
					<div class="title_line">
						<h1><spring:message code="appvl.sidemenu.tree.processing.ongoing"/></h1>						
					</div>					
				</div>
				<!-- paging -->
				...
				<!-- List Start -->	
					<table summary="/No/Attochment/Title/Poster/Post Date/Expire date" class="board_type_height">
						...
						<tbody>
							...
							<c:forEach var="ongoing" items="${ongoingList}">
							<tr>
								<td>
									<c:if test="${ongoing.docType == 'DT01'}"><spring:message code="appvl.doctype.internal"/></c:if>
									<c:if test="${ongoing.docType == 'DT02'}"><spring:message code="appvl.doctype.outgoing"/></c:if>
									<c:if test="${ongoing.docType == 'DT03'}"><spring:message code="appvl.doctype.incoming"/></c:if>
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${ongoing.draftDateTime }" />
								</td>
								<td class=""><a href="javascript:approvalView('<c:out value="${ongoing.docID}"/>')">${ongoing.docTitle}</a></td>
								<td>${ongoing.draftDeptName }</td>
								<td>${ongoing.drafterName }</td>
								<td><a href="javascript:status('<c:out value="${ongoing.docID}"/>', '<c:out value="${ongoing.docTitle}"/>')" class="Status">
									<input type="hidden" value="${ongoing.docID}" id="ongoingDocId" name="ongoingDocId">
									<c:if test="${ongoing.docPState =='DP00'}"><spring:message code="appvl.docpState.DP00"/></c:if>
									<c:if test="${ongoing.docPState =='DP01'}"><spring:message code="appvl.docpState.DP01"/></c:if>
									<c:if test="${ongoing.docPState =='DP02'}"><spring:message code="appvl.docpState.DP02"/></c:if>
									<c:if test="${ongoing.docPState =='DP09'}"><spring:message code="appvl.docpState.DP09"/></c:if>
								</a></td>
								<!-- 첨부파일이 없을때 해당 a태그에 'hidden' class 추가 -->
								<td class="align_center">
									<c:if test="${ongoing.docAttaF =='1'}">
										<a href="javascript:fileDown('<c:out value="${ongoing.docID}"/>', '<c:out value="${ongoing.docTitle}"/>')" class="fileDown">
											<input type="hidden" value="${ongoing.docID}" id="ongoingDocId" name="ongoingDocId">
