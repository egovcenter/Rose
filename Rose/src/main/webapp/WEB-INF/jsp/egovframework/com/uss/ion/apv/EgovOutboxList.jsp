			...
			<div class="content_box"> 
				...
				<div class="title_box">
					<div class="title_line">
						<h1><spring:message code="appvl.sidemenu.tree.cabinet.outbox"/></h1>						
					</div>					
				</div>
				...
				<!-- List Start -->	
					<table summary="/No/Attochment/Title/Poster/Post Date/Expire date" class="board_type_height">
						...
						<thead>
							<tr>
								<th scope="col" onclick="javascript:fnOrderList(this, 1);" class='<c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '1' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.outbox.table.draftdate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 2);" class='<c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '2' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.outbox.table.finisheddate"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 3);" class='<c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '3' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.outbox.table.title"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 4);" class='<c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '4' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.outbox.table.draftdept"/></span></th>
								<th scope="col" onclick="javascript:fnOrderList(this, 5);" class='<c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'asc' }">sort_up</c:if><c:if test="${page.orderColumn eq '5' && page.orderMethod eq 'desc' }">sort_down</c:if>'><span><spring:message code="appvl.cabinet.outbox.table.drafter"/></span></th>
						...
						<tbody>
							...
							<c:forEach var="outbox" items="${outboxList}">
							<tr>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${outbox.draftDateTime }" />
								</td>
								<td class="">
									<fmt:formatDate pattern="yyyy-MM-dd" value="${outbox.approvedDateTime }" />
								</td>
								<td><a href="javascript:approvalView('<c:out value="${outbox.docID}"/>')">${outbox.docTitle}</a></td>
								<td>${outbox.draftDeptName}</td>
								<td>${outbox.drafterName}</td>
								<td class="align_center">
									<a href="javascript:status('<c:out value="${outbox.docID}"/>', '<c:out value="${outbox.docTitle}"/>')" class="Status">
									<img src="<c:url value='/images/egovframework/com/uss/cmm/icon_history.png'/>" alt="icon_history"></a>
								</td>
								<td class="align_center">
									<c:if test="${outbox.docAttaF =='1'}">
										<a href="javascript:fileDown('<c:out value="${outbox.docID}"/>', '<c:out value="${outbox.docTitle}"/>')" class="fileDown">
											<input type="hidden" value="${outbox.docID}" id="ongoingDocId" name="ongoingDocId">
