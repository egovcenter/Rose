...
var orgPopup = {
	...
	initOrgTree : function(isExpandAll) {
		...
		$("#orgTree").dynatree("getRoot").appendAjax({
			url: CONTEXT+"/initDeptTree.do",
			type: "post",
			data: {baseDept: $("#baseDept").val(), startDept: $("#startDept").val(), isExpandAll: isExpandAll },
	...
	viewUserList : function(deptId) {
		$("#userList").load(CONTEXT + "/userList.do", {
			action: "getUserInfoForPopup",
			checkType: "multi",
			display: $("#display").val(),
...
/* dirGroup : end */
	...
	onLoad : function(to, cc, bcc) {
		...
		if ($("#checkbox").val() == "tree") {
			...
			resizePopup(900, $("#pop_wrap").height());
		}
