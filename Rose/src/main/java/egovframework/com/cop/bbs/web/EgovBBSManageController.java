...
import egovframework.com.cmm.TreeNodeVO;
...
public class EgovBBSManageController {
	...
	List<TreeNodeVO> treeList = bbsAttrbService.selectBBSTreeList();
	
	String bbsId=request.getParameter("nodeId");
	String bbsNm=request.getParameter("nodeNm");
	...
	if(bbsId==null||bbsId.equals(""))
	{
		...
		{
			if(treeList.size() > 0){
				boardVO.setBbsId(treeList.get(0).getNodeId()); 
				boardVO.setBbsNm(treeList.get(0).getNodeNm());
			}
		}else{
			boardVO.setBbsId(boardVO.getBbsId());
			boardVO.setBbsNm(boardVO.getBbsNm());
	...
	if(vo.getBbsId().length() > 0){
		...
		model.addAttribute("resultCnt", null);
		model.addAttribute("boardVO", boardVO);
		model.addAttribute("brdMstrVO", null);
		model.addAttribute("BbsEempty", true);
	}
	...
	if (isAuthenticated) {
	    ...
	    board.setNtcrId(user.getEmplyrId()); //게시물 통계 집계를 위해 등록자 ID 저장
	    board.setNtcrNm(user.getEmplyrNm()); //게시물 통계 집계를 위해 등록자 Name 저장
	...
	boardVO.setBbsId(boardVO.getBbsId());
	boardVO.setBbsNm(boardVO.getBbsNm());
	boardVO.setNtcrNm(user.getEmplyrNm());
